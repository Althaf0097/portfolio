...
const generateParticleData = (count) => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    // Random offsets for individual particle animation
...
const ParticleField = ({ count = 1000, mousePositionRef, scrollProgressRef }) => {
  ...
  useFrame((state) => {
    ...
    // Move the entire particle field vertically based on scroll for parallax effect
    mesh.current.position.y = scroll * 20;
  });
      ...
      <shaderMaterial
        ...
        vertexShader={`
          ...
          void main() {
            ...
            float floatX = sin(uTime * 0.2 + offset.x) * 0.15;
            float floatY = cos(uTime * 0.15 + offset.y) * 0.2;
            float floatZ = sin(uTime * 0.18 + offset.z) * 0.1;
...
const FloatingShapes = ({ scrollProgressRef }) => {
  ...
  useFrame((state) => {
    ...
    groupRef.current.position.y = (scrollProgressRef.current || 0) * 10;
  });
...
const AnimatedShape = ({ pos, scale, color, index, scrollProgressRef }) => {
  ...
  useFrame((state) => {
    ...
    meshRef.current.rotation.x = time * (0.2 + index * 0.05) + scroll * 2;
