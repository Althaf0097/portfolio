...
// Pure helper to generate random particle data to satisfy React 19 rules
...
const ParticleField = ({ count = 1000, mousePositionRef, scrollProgressRef }) => {
  ...
  useFrame((state) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      // Add scroll-based vertical offset for parallax effect
      const scrollOffset = scrollY * 20;
...
const FloatingShapes = ({ scrollProgressRef }) => {
  ...
  useFrame((state) => {
    ...
    // Parallax scroll effect for the whole group based on scroll ref
    groupRef.current.position.y = scrollY * 15;
  });
...
const Scene3DFullPage = () => {
  ...
  // High-frequency values moved to refs to prevent re-renders
  const mousePositionRef = useRef({ x: 0, y: 0 });
  const scrollProgressRef = useRef(0);

  // Lazy initialization of isMobile to avoid extra render on mount
