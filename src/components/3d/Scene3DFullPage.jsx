 ...
 * Extracted from component to satisfy React 19 purity rules and optimize performance.
 */
 ...
 * uses GPU-accelerated vertex shaders for O(1) CPU overhead.
 */
const ParticleField = ({ count = 1000, mousePosition, scrollProgress }) => {
  ...
  useFrame((state) => {
    ...
    materialRef.current.uniforms.uScroll.value = scrollProgress?.current || 0;
  });
  ...
  const shaderArgs = useMemo(() => ({
    ...
    vertexShader: `
      ...
      void main() {
        ...
        // Base floating motion calculated on GPU
        float floatX = sin(uTime * 0.2 + aRandomOffset) * 0.15;
        float floatY = cos(uTime * 0.15 + aRandomOffset * 1.5) * 0.2;
        float floatZ = sin(uTime * 0.18 + aRandomOffset * 1.2) * 0.1;

        pos.x += floatX + uMouse.x * 0.3;
        pos.y += floatY + uMouse.y * 0.3 + (uScroll * 30.0); // Parallax scroll effect
        pos.z += floatZ;
 ...
 */
const FloatingShapes = ({ scrollProgress }) => {
  ...
  useFrame((state) => {
    ...
    groupRef.current.position.y = (scrollProgress?.current || 0) * 30.0;
  });
