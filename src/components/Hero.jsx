...
const Hero = () => {
  ...
  useEffect(() => {
    const handleMouseMove = (e) => {
      ...
      // Directly update DOM style to avoid React re-renders on every mouse move
      // This reduces re-renders from O(N) to 0 during mouse interaction
      glowRef.current.style.background = `radial-gradient(circle at ${x}% ${y}%, rgba(59, 130, 246, 0.08) 0%, transparent 40%)`;

      // Ensure the glow is visible after the first movement
  ...
  return (
    <section className="relative min-h-[90vh] flex items-center justify-center overflow-hidden pt-20 bg-dark-950">
      {/* Volumetric Mouse Follow Glow - Optimized via direct DOM updates */}
      <div 
        ref={glowRef}
