...
// BOLT OPTIMIZATION: Move non-deterministic random position generation outside of component render scope to satisfy purity rules
...
const CameraController = ({ mouse }) => {
  useFrame((state) => {
    // BOLT OPTIMIZATION: Destructure camera from state in useFrame to satisfy React 19 immutability guidelines
    const { camera } = state;
