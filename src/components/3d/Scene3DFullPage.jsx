...
// Helper for random generation outside component to satisfy React 19 purity rules
const generateEnhancedParticles = (count) => {
...
const ParticleField = ({ count = 1000, mousePosition, scrollProgress }) => {
  ...
  // Use helper for random generation to satisfy purity rules
  const particles = useMemo(() => generateEnhancedParticles(count), [count]);
  ...
  useFrame((state) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      // Combine mouse movement with scroll for depth parallax
      const scrollY = (scrollProgress?.current || 0) * 10;
...
const FloatingShapes = ({ scrollProgress }) => {
  ...
  useFrame((state) => {
    ...
    // Use scroll progress for group vertical parallax
    const scroll = scrollProgress?.current || 0;
    groupRef.current.position.y = scroll * 20;
  });
...
const Scene3DFullPage = () => {
  ...
  // Using refs instead of state to prevent re-renders on every mouse move or scroll
