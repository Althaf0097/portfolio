...
const InteractiveCard = ({
  ...
  // GSAP quickTo helpers for ultra-smooth 60fps+ tracking without layout thrashing
  const cardRotateX = useRef(null);
  const cardRotateY = useRef(null);
  ...
  useEffect(() => {
    if (cardRef.current) {
      cardRotateX.current = gsap.quickTo(cardRef.current, 'rotateX', { duration: 0.35, ease: 'power2.out' });
      cardRotateY.current = gsap.quickTo(cardRef.current, 'rotateY', { duration: 0.35, ease: 'power2.out' });
  ...
  const handleMouseMove = (e) => {
    ...
    // Calculate page-relative mouse coordinates to avoid layout thrashing via getBoundingClientRect
    ...
    // BOLT OPTIMIZATION: GSAP quickTo handles hardware-accelerated transforms and avoids main-thread overhead
    if (cardRotateY.current) cardRotateY.current(normalizedX * 12);
    if (cardRotateX.current) cardRotateX.current(-normalizedY * 12);
    if (cardY.current) cardY.current(-16);
    if (cardScale.current) cardScale.current(1.03);

    // Spotlight glare translation using transform: translate3d
    if (glareX.current && glareY.current && glareOpacity.current) {
      glareX.current(x - 350);
      glareY.current(y - 350);
      glareOpacity.current(1);
    }
  };

  const handleMouseEnter = (e) => {
    // BOLT OPTIMIZATION: Cache card dimensions on hover enter to completely eliminate getBoundingClientRect in mouse move
    if (cardRef.current) {
  ...
  return (
    ...
    >
      ...
      {/* BOLT OPTIMIZATION: Glare overlay translated on GPU via transform: translate3d. Set top-0 left-0 explicitly to prevent layout shifting. */}
      <div
        ref={glareRef}
        className="pointer-events-none absolute top-0 left-0 w-[700px] h-[700px] opacity-0 z-10 mix-blend-screen rounded-full"
        style={{
