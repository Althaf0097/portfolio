...
// BOLT OPTIMIZATION: Move impure Math.random call to a static helper function outside of the component body to comply with React 19 rules and avoid layout recalculation on every render pass.
...
const FloatingParticles = ({ count = 300 }) => {
  ...
  // BOLT OPTIMIZATION: Memoize particle positions array to prevent expensive recreation and memory allocations on every re-render.
  const positions = useMemo(() => generateParticles(count), [count]);

  // BOLT OPTIMIZATION: Memoize CanvasTexture generation and properly dispose of it to completely eliminate severe memory leaks and rendering lag.
...
const CameraController = ({ mouse }) => {
  // BOLT OPTIMIZATION: Read camera from the useFrame callback parameter instead of hook-based useThree to comply with React 19 immutability guidelines.
  useFrame(({ camera }) => {
