...
// BOLT OPTIMIZATION: Extract particle position generation outside the component body
// to avoid impure Math.random calls during render and satisfy React 19 rules.
...
const FloatingParticles = ({ count = 300 }) => {
  ...
  // BOLT OPTIMIZATION: Memoize particle positions using the static helper function
...
const CameraController = ({ mouse }) => {
  useFrame((state) => {
    // BOLT OPTIMIZATION: Read camera from useFrame state argument instead of direct useThree hook to respect React 19 immutability guidelines
    const { camera } = state;
