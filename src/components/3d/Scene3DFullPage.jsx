...
/**
 * Data generation helper moved outside component to ensure React 19 purity
 */
...
const ParticleField = ({ count = 1000, mousePositionRef, scrollProgressRef }) => {
  ...
  useFrame((state) => {
    ...
    const scrollOffset = (scrollProgressRef?.current || 0) * 40;
    ...
    for (let i = 0; i < count; i++) {
      ...
      // Apply scroll-based vertical offset
      positions[i3 + 1] = initialPositions[i3 + 1] + floatY + mouseY * 0.3 + scrollOffset;
...
const FloatingShapes = ({ scrollProgressRef }) => {
  ...
  useFrame((state) => {
    ...
    const time = state.clock.getElapsedTime();
    const scrollOffset = (scrollProgressRef?.current || 0) * 15;
    groupRef.current.rotation.y = time * 0.03 + scrollOffset * 0.1;
...
const Scene3DFullPage = () => {
  ...
  // This reduces re-renders from ~60fps to 0 during mouse/scroll interaction
  const mousePositionRef = useRef({ x: 0, y: 0 });
  const scrollProgressRef = useRef(0);

  // Optimization: initialize state lazily to avoid double render on mount
