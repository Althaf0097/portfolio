...
const FloatingParticles = ({ count = 300 }) => {
  ...
  // BOLT OPTIMIZATION: Memoize particle positions array to prevent re-creating Float32Array on every render
  const positions = useMemo(() => generateParticlePositions(count), [count]);

  // BOLT OPTIMIZATION: Memoize particle CanvasTexture to avoid allocating canvas element & texture GPU memory on every render
