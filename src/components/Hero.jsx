...
const Hero = () => {
  ...
  useEffect(() => {
    const handleMouseMove = (e) => {
      ...
      if (glowRef.current) {
        // Direct DOM update to bypass React render cycle for high-frequency mouse movement
  ...
  useEffect(() => {
    // Initialize background on mount to satisfy React 19 ref access rules
    if (glowRef.current) {
      glowRef.current.style.background = `radial-gradient(circle at ${mousePosRef.current.x}% ${mousePosRef.current.y}%, rgba(59, 130, 246, 0.08) 0%, transparent 40%)`;
    }
  }, []);
