...
/**
 * Helper to generate initial particle data outside the component scope
 * to satisfy React 19 purity rules for deterministic rendering.
 */
...
const ParticleField = ({ count = 1000, mousePositionRef, scrollProgressRef }) => {
  ...
  useFrame((state) => {
    ...
    const mouseY = (mousePositionRef?.current?.y || 0) + (scrollProgressRef?.current || 0) * 5;
...
const FloatingShapes = ({ scrollProgressRef }) => {
  ...
  // Static shape positions - no Math.random() here but good to keep static
