...
const particleVertexShader = `
  ...
  void main() {
    ...
    pos.x += sin(time + position.z) * 0.15;
    pos.y += cos(time * 0.75 + position.x) * 0.2;
    pos.z += sin(time * 0.9 + position.y) * 0.1;

    // Mouse parallax
    pos.x += uMouse.x * 0.3 * (1.0 + position.z * 0.05);
    pos.y += uMouse.y * 0.3 * (1.0 + position.z * 0.05);

    // Scroll parallax (vertical offset based on scroll)
    pos.y += uScroll * 15.0;
