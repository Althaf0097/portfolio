...
// Helper to generate particle data outside the component to satisfy React 19 purity rules
 ...
 */
const ParticleField = ({ count = 1000, mousePosition, scrollProgressRef }) => {
  const mesh = useRef();
  ...
  useFrame((state) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      // Add scroll-based vertical shift for parallax effect
...
const FloatingShapes = ({ scrollProgressRef }) => {
  ...
  useFrame((state) => {
    ...
    // Apply scroll parallax to the entire group
