...
// Pure data generation helper to satisfy React 19 purity rules
...
const ParticleField = ({ count = 1000, mousePositionRef, scrollProgressRef }) => {
  ...
  useFrame((state) => {
    ...
    mesh.current.position.y = scrollY * 10;
...
const FloatingShapes = ({ scrollProgressRef }) => {
  ...
  useFrame((state) => {
    ...
    // Apply vertical parallax based on scroll
    groupRef.current.position.y = (scrollProgressRef?.current || 0) * 15;
  });
...
const Scene3DFullPage = () => {
  ...
  // Ref-based tracking to avoid React re-renders on high-frequency events (mouse/scroll)
  // This saves ~100+ re-renders per second during active interaction
