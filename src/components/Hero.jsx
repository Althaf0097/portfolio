...
const Hero = () => {
  ...
  // Update glow style directly to avoid React re-renders on every mouse move
  const updateGlowStyle = (x, y) => {
    if (glowRef.current) {
  ...
  // Re-apply glow style after React re-renders (e.g. when role changes)
  useEffect(() => {
    updateGlowStyle(mousePosRef.current.x, mousePosRef.current.y);
  });
  ...
  return (
    <section className="relative min-h-[90vh] flex items-center justify-center overflow-hidden pt-20 bg-dark-950">
      {/* Volumetric Mouse Follow Glow - Optimized with useRef to avoid re-renders */}
      <div 
        ref={glowRef}
