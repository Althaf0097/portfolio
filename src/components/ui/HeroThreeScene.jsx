...
// Helper function to generate static deterministic values outside the component render loop
// to satisfy React 19/purity and ESLint rules.
const generateParticles = (count) => {
  const positions = new Float32Array(count * 3);
  for (let i = 0; i < count; i++) {
    // Generate static values (will be memoized)
...
const FloatingParticles = ({ count = 300 }) => {
  ...
  // Pure/Idempotent rendering: generate positions once or memoize them
  const positions = useMemo(() => generateParticles(count), [count]);

  // BOLT OPTIMIZATION: Memoize canvas texture generation to prevent memory leaks and GC overhead
...
const CameraController = ({ mouse }) => {
  useFrame((state) => {
    // BOLT OPTIMIZATION: Destructure camera from state inside useFrame
    // rather than calling useThree hook directly, to comply with React 19 rules.
    const { camera } = state;
