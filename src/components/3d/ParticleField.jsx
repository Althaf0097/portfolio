...
/**
 * Helper to generate random particle data outside component to satisfy React 19 purity rules
 */
 ...
 */
const ParticleField = ({ count = 800, mousePosition, mousePositionRef }) => {
  const mesh = useRef();
  ...
  useFrame((state) => {
    ...
    // Supports both direct value and ref for flexible optimization
    const mouseX = mousePositionRef?.current?.x ?? mousePosition?.x ?? 0;
    const mouseY = mousePositionRef?.current?.y ?? mousePosition?.y ?? 0;
