...
// Helper to generate random particle data outside of component to maintain purity
...
const vertexShader = `
  ...
  void main() {
    ...
    // Replicate floating motion on GPU
    float floatX = sin(uTime * 0.2 + aOffset * 0.1) * 0.15;
    float floatY = cos(uTime * 0.15 + aOffset * 0.15) * 0.2;
    float floatZ = sin(uTime * 0.18 + aOffset * 0.12) * 0.1;

    // Add scroll parallax (moving particles up as we scroll down)
    float scrollOffset = uScroll * 10.0;

    pos.x += floatX + uMouse.x * 0.3;
    pos.y += floatY + uMouse.y * 0.3 + scrollOffset;
    pos.z += floatZ;

    vec4 mvPosition = modelViewMatrix * vec4(pos, 1.0);
    gl_Position = projectionMatrix * mvPosition;

    // Size attenuation - adjusted to match original look
...
const FloatingShapes = ({ scrollProgress }) => {
  ...
  useFrame((state) => {
    ...
    // Add vertical parallax for shapes
    groupRef.current.position.y = scrollProgress.current * 15.0;
  });
