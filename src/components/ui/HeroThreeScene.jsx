...
// BOLT OPTIMIZATION: Move non-deterministic geometry generations outside
// the render tree / component functions to satisfy React 19's strict purity guidelines.
...
const FloatingParticles = ({ count = 300 }) => {
  ...
  // BOLT OPTIMIZATION: Memoize the CanvasTexture generation to prevent severe WebGL memory leaks
  // and redundant texture instantiation on re-renders, and properly dispose of it on unmount.
...
const CameraController = ({ mouse }) => {
  useFrame((state) => {
    // BOLT OPTIMIZATION: To satisfy React 19 rules, destructure camera from the useFrame state object
    // and modify camera.position in frame updates rather than fetching hooks or editing immutable structures.
    const { camera } = state;
