...
const ParticleField = ({ count = 1000, mousePositionRef, scrollProgressRef }) => {
  ...
  useFrame((state) => {
    ...
    const scrollY = (scrollProgressRef?.current || 0) * 20;
...
const FloatingShapes = ({ scrollProgressRef }) => {
  ...
  useFrame((state) => {
    ...
    groupRef.current.position.y = (scrollProgressRef?.current || 0) * 20;
  });
...
const Scene3DFullPage = () => {
  ...
  // BOLT OPTIMIZATION: Use refs for high-frequency updates (mouse/scroll)
  // to prevent unnecessary React re-renders during interaction.
