...
// Helper function to generate particle positions outside the render function
// to maintain component purity and prevent recreation on each frame.
...
const FloatingParticles = ({ count = 300 }) => {
  ...
  // BOLT OPTIMIZATION: Memoize positions calculation to avoid recreation and satisfy React purity.
  const positions = useMemo(() => generateParticles(count), [count]);

  // BOLT OPTIMIZATION: Memoize canvas texture generation to prevent severe memory leaks and garbage collection bottlenecks in Three.js/React Three Fiber.
...
const CameraController = ({ mouse }) => {
  // BOLT OPTIMIZATION: Destructure camera from state argument inside useFrame to avoid direct mutation of a hook value.
  useFrame((state) => {
    const { camera } = state;
