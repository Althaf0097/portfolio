...
// Pure particle position generator function outside component to satisfy React 19 component purity rules
...
const FloatingParticles = ({ count = 300 }) => {
  ...
  // BOLT OPTIMIZATION: Memoize particle positions to avoid re-generating positions on re-renders
  const positions = useMemo(() => generateParticlePositions(count), [count]);

  // BOLT OPTIMIZATION: Memoize canvas texture to prevent creating canvas elements and Three.js textures on re-renders, preventing memory leaks and GC overhead
...
const CameraController = ({ mouse }) => {
  // Destructure camera from state argument inside useFrame to satisfy React 19 immutability rules
  useFrame(({ camera }) => {
