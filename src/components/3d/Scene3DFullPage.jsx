...
// BOLT: Move particle generation outside to satisfy react-hooks/purity
...
const ParticleField = ({ count = 1000, mousePositionRef, scrollProgressRef }) => {
  ...
  useFrame((state) => {
    ...
    // BOLT OPTIMIZATION: Read from refs to avoid React re-renders on every mouse move/scroll
    ...
    for (let i = 0; i < count; i++) {
      ...
      // Parallax effect using scroll progress
      const scrollOffset = scroll * 10;
...
const FloatingShapes = ({ scrollProgressRef }) => {
  ...
  useFrame((state) => {
    ...
    // BOLT OPTIMIZATION: Apply parallax based on scroll without re-rendering
    const scroll = scrollProgressRef?.current ?? 0;
...
const Scene3DFullPage = () => {
  ...
  // BOLT OPTIMIZATION: Use refs for high-frequency updates to eliminate React re-renders
