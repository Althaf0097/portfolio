...
/**
 * Helper function for generating particle data to ensure React component purity.
 * Moves non-deterministic Math.random() calls outside the component scope.
 */
...
const ParticleField = ({ count = 1000, mousePositionRef, scrollProgressRef }) => {
  ...
  useFrame((state) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const parallaxY = scrollY * 15;

      positions[i3] = initialPositions[i3] + floatX + mouseX * 0.3;
      positions[i3 + 1] = initialPositions[i3 + 1] + floatY + mouseY * 0.3 - parallaxY;
...
const AnimatedShape = ({ pos, scale, color, index, scrollProgressRef }) => {
  ...
  useFrame((state) => {
    ...
    // Parallax effect: shapes move slightly with scroll
    const parallaxY = scrollY * 10;
    meshRef.current.position.y = pos[1] + Math.sin(time * 0.5 + index) * 0.3 - parallaxY;
  });
