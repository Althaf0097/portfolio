 ...
 */
const generateFullPageParticles = (count) => {
...
const vertexShader = `
  ...
  void main() {
    ...
    // Base floating motion on GPU
    float floatX = sin(uTime * 0.2 + position.x * 0.1) * 0.15;
    float floatY = cos(uTime * 0.15 + position.y * 0.15) * 0.2;
    float floatZ = sin(uTime * 0.18 + position.z * 0.12) * 0.1;
...
const ParticleField = ({ count = 1000, mousePositionRef, scrollProgressRef }) => {
  ...
  const particles = useMemo(() => generateFullPageParticles(count), [count]);
