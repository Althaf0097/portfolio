...
const ACCENT_COLOR_FULL = new THREE.Color('#3b82f6');
const BLUE_COLOR_FULL = new THREE.Color('#06b6d4');
const WHITE_COLOR_FULL = new THREE.Color('#ffffff');

const generateParticlesFull = (count) => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    let color;
    if (colorChoice < 0.4) color = ACCENT_COLOR_FULL;
    else if (colorChoice < 0.7) color = BLUE_COLOR_FULL;
    else color = WHITE_COLOR_FULL;
...
const ParticleField = ({ count = 1000, mousePosition }) => {
  ...
  const particles = useMemo(() => generateParticlesFull(count), [count]);
