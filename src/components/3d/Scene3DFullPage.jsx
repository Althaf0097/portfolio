...
// Define helper functions outside component scope for React 19 purity rules
const generateParticleData = (count) => {
  const positions = new Float32Array(count * 3);
  const colors = new Float32Array(count * 3);
  const randoms = new Float32Array(count * 3); // For unique particle motion
 ...
 * Movement and parallax handled in vertex shader for maximum performance
 */
const ParticleField = ({ count = 1000, mousePositionRef, scrollProgressRef }) => {
  const materialRef = useRef();

  const { positions, colors, randoms } = useMemo(() => generateParticleData(count), [count]);
      ...
      <shaderMaterial
        ...
        vertexShader={`
          ...
          void main() {
            ...
            // Floating motion
            pos.x += sin(uTime * 0.2 + randoms.x * 10.0) * 0.15 + uMouse.x * 0.3;
            pos.y += cos(uTime * 0.15 + randoms.y * 15.0) * 0.2 + uMouse.y * 0.3 + uScroll * 10.0;
            pos.z += sin(uTime * 0.18 + randoms.z * 12.0) * 0.1;
...
const FloatingShapes = ({ scrollProgressRef }) => {
  ...
  useFrame((state) => {
    ...
    groupRef.current.position.y = scrollProgressRef.current * 10.0;
  });
